...
async function createAPI(pyodide: PyodideInterface) {
    ...
    wrapped_api
    `);
    console.log("api created in worker", api, Object.keys(api));
    console.log("wrapped_api:", pyodide.globals, Object.keys(pyodide.globals));
    return api;
}
...
export class Server {
    ...
    async init() {
        ...
        await this.asyncEmit("server_startup_status", {status: "loading bumps", percent: 75});
